...
for (const nBlocks of [1, 2, 3]) {
  for (const nHeads of [1, 4]) {
    const cfg = { vocab: tok.vocab, dEmbed: 16, dFF: 32, blockSize: 8, nHeads, nBlocks };
    ...
    console.log(
      `nBlocks=${nBlocks} nHeads=${nHeads}  params=${model.params().length}  checked=${checked}  worst rel=${worst.toExponential(2)}  -> ${worst < 2e-2 ? 'PASS' : 'FAIL'}`,
    );
  }
}
